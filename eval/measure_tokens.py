...
def slice_for(pdf: Path, question: str) -> str:
    ...
    proc = subprocess.run(
        [sys.executable, str(RETRIEVE), str(pdf), "--query", question,
         "--top-k", "5", "--neighbors", "1", "--anchors", "--follow-xrefs", "--passages"],
...
def ask(client, context: str, question: str, model: str = MODEL):
...
def count_input(client, context: str, question: str, model: str = MODEL) -> int:
...
def main():
    ...
    for q in questions:
        ...
        if args.dry_run:
            ...
            ni = count_input(client, whole, q, args.model)
            si = count_input(client, sl, q, args.model)
            naive_in += ni; skill_in += si
            print(f"  naive input {ni} tok   skill input {si} tok   "
                  f"(slice = {si * 100 // max(ni,1)}% of naive)")
        else:
            na, ni, no = ask(client, whole, q, args.model)
            sa, si, so = ask(client, sl, q, args.model)
